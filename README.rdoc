...
This package has been tested with Ruby 1.9.3. It requires Sqlite3 database
with Spatialite extensions.

For more information on setting up Spatialite on your system, see:
https://github.com/chip-rosenthal/findit-support#spatialite-support
...
There is nothing in the application or libraries that needs to be changed when
porting to another location or updating for another election.

What's required for porting or updating is:

* Generate a new database.
* Update the "public/about.html" page.

The database must be regenerated for each election. See the "data" directory
for the source data used to build the database.

See the data/README.txt for information on database generation.
